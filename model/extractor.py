...
class MonoModalMultiHeadAttention(nn.Module):
    def __init__(self, embedding, num_heads):
        super().__init__()
        self.d_model = embedding
        self.num_heads = num_heads
    ...
    def forward(self, query, key, value, mask=None):
        ...
        if mask is not None:
            print(f'mask: {mask.shape}')
            attn_mask = torch.logical_not(mask.unsqueeze(1).unsqueeze(2))
            print(f'attn_mask: {attn_mask.shape}')
            attention_score = attention_score.masked_fill_(attn_mask, -np.inf)
...
class LocalStackExtractor(Extractor):
    """Self-attention based local feature extractor"""
    ...
    def __init__(self, **kwargs):
        ...
        self.speech_attn = MonoModalMultiHeadAttention(self.embedding, self.num_heads)
        self.text_attn = MonoModalMultiHeadAttention(self.embedding, self.num_heads)
    ...
    def forward(self, emb_s, emb_t, emb_s_mask=None, emb_t_mask=None, verbose=False):
        ...
        Args:
            ...
            emb_t   : text embedding of shape `(batch, phoneme, embedding)`
        Returns:
            enhanced_emb_s : locally attended speech embedding
            enhanced_emb_t : locally attended text embedding
        """

        # Speech self-attention
        enhanced_emb_s, _ = self.speech_attn(
            query=emb_s, 
            key=emb_s, 
            value=emb_s,
            mask=emb_s_mask
        ...
        # Text self-attention
        enhanced_emb_t, _ = self.text_attn(
            query=emb_t, 
            key=emb_t, 
            value=emb_t,
            mask=emb_t_mask
        ...
        return enhanced_emb_s, enhanced_emb_t


class HierarchicalExtractor(Extractor):
    """Hierarchical attention extractor: Local feature attention -> Joint feature attention"""
    ...
    def __init__(self, **kwargs):
        ...
        # Local attention
        self.local_extractor = LocalStackExtractor(
            embedding=self.embedding,
            num_heads=self.num_heads
        )

        # Joint attention
        self.joint_extractor = StackExtractor(
            embedding=self.embedding,
            num_heads=self.num_heads
        )
    ...
    def forward(self, emb_s, emb_t, emb_s_mask=None, emb_t_mask=None, verbose=False):
        ...
        Args:
            ...
            emb_t   : text embedding of shape `(batch, phoneme, embedding)`
        Returns:
            attention_output : joint-attended embedding
            affinity_matrix  : joint attention map
            attn_mask        : mask for attention output
            affn_mask        : mask for affinity matrix
        """

        # Step 1: Local Feature Attention
        enhanced_emb_s, enhanced_emb_t = self.local_extractor(
            emb_s, emb_t, emb_s_mask, emb_t_mask, verbose
        )

        # Step 2: Joint Attention
        attention_output, affinity_matrix, attn_mask, affn_mask = self.joint_extractor(
            enhanced_emb_s, enhanced_emb_t, emb_s_mask, emb_t_mask, verbose
        )
